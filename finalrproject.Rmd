...
ggplot() + geom_polygon(data = usa, aes(x=long, y = lat, group = group)) + 
  coord_fixed(1.3) +
    geom_point( aes(x = longi, y = lati, color=factor(rtwl_data$RTWL), size=rtwl_data$COL))
```
```{r}
usa <- map_data("usa") # we already did this, but we can do it again
ggplot() + geom_polygon(data = usa, aes(x=long, y = lat, group = group)) + 
  coord_fixed(1.3) +
    geom_point( aes(x = longi, y = lati, color=(rtwl_data$Income), size=rtwl_data$COL))
```
...
```{r}
library(corrplot)
correlations <- cor(rtwl_data[,1:9])
corrplot(correlations, method="circle")
pairs(rtwl_data[,c(1,2,3,6,7)], col=factor(rtwl_data$RTWL))
```


```{r}
l_model <- glm(factor(RTWL) ~ COL + Income, data = rtwl_data, family = binomial)
```
```{r}
summary(l_model)
```
